...
def convert_coco_json(json_dir='./datasets/coco/annotations/', output_dir='./datasets/coco/labels/',
    ...
    for json_file in sorted(jsons):
        ...
        for x in tqdm(data['annotations'], desc='Annotations %s' % json_file, position=0, leave=True):
            ...
            h, w, f = img['height'], img['width'], img['file_name'].split(
                "/")[-1]
            ...
            if box[2] > 0 and box[3] > 0:  # if w > 0 and h > 0
                line = x['category_id'], * \
                    (s if use_segments else box)  # cls, box or segments
        ...
        for file in filenames:
            task = MyThread(
                file + "\n", os.path.join(text_dir, sub_type + ".txt"))
            tasks.append(task)
            task.start()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--json-dir', type=str,
                        default='.\\datasets\\coco\\annotations\\', help='jsons folder path')
    parser.add_argument('--output-dir', type=str,
                        default='.\\datasets\\coco\\labels\\', help='labels output folder path')
    parser.add_argument('--image-dir', type=str,
                        default='.\\datasets\\coco\\data\\', help='labels output folder path')
    parser.add_argument('--text-dir', type=str,
                        default='.\\datasets\\coco\\', help='labels output folder path')
    args = parser.parse_args()

    convert_coco_json(args.json_dir, args.output_dir,
                      args.image_dir, args.text_dir)
